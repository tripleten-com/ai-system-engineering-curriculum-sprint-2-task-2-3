...
Purpose:           Validate the Task 2.3 answer sheet and the permitted change paths.
...
ALLOWED_PATHS = frozenset(
    {
        ...
        # The one file inside src/adapters/ that any Task permits a student to
        # change. Persistence needs a concrete driver, and `domain` may not
        # import one, so the data layer is implemented here.
...
def main(root: Path | None = None, *, changed_paths: list[str] | None = None) -> int:
    ...
    try:
        validate_submission(
            ...
            # No sample comparison for this Task. The accepted answer sheet IS
            # the empty mapping the sample shows, so a copy check would reject
        ...
        return 1
    print("Task 2.3 answer verification passed.")
    return 0
...
def validate_submission(
    ...
    *,
    ...
    # Task 2.3 has no useful direct answer: the work is an implementation, and
    # the checks read the running system rather than a claim about it. The
    # mapping is therefore required to be present and empty, which is a real
    # check - it rejects an invented status field as firmly as a missing one.
    if answers:
        raise SubmissionError(
            "answers must stay an empty mapping for this Task; the implementation and its "
            f"automated checks are the deliverable, not a recorded claim (found: "
